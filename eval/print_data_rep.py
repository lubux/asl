...
result_mw = mw_res_dict[rep_factor]

mw_time = result_mw[:,1] + result_mw[:,2]

tmp = result_tp[:, 3]/1000
for ind in range(3):
    string_name = "%s, %d servers" % (rep_factros[0], result_tp[ind, 0])
    print "\hline " + string_name + " & " + "%.0f" % result_tp[ind, 1] + " & %.2f" % tmp[ind] + " & %.2f" % mw_time[ind] + " & %.2f" % result_mw[ind, 2] + " & %.2f" % result_mw[ind, 3] + " \\\\"
