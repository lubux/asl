...
OUTPUT_DIR="repexp"
WORKLOAD_FILE="replication_experiment.cfg"
...
do
	...
	do
		DIVIDE=$(($NUM_SERVERS / 2 + 1))
		for REPLICATION in {1,$DIVIDE,$NUM_SERVERS}
		do
			...
			do
				...
				MEMASLAP_CMD="./libmemcached-1.0.18/clients/memaslap -s $MW_MACHINE_P:$SERVER_PORT -T $NUM_CLIENTS -c $NUM_CLIENTS -o 0.9 -S 1s -t $NUM_MINUTES -F $WORKLOAD_FILE > clientlog_$NUM_SERVERS-$COUNTER-$ROUND-$REPLICATION.txt"
				echo $MEMASLAP_CMD
			...
			for client_machine in $CLIENT_MACHINES
			do
				scp $USER_MACHINES@$client_machine:clientlog_$NUM_SERVERS-$COUNTER-$ROUND-$REPLICATION.txt ../data/$OUTPUT_DIR/
				ssh $USER_MACHINES@$client_machine "rm clientlog_$NUM_SERVERS-$COUNTER-$ROUND-$REPLICATION.txt"
				COUNTER=$((COUNTER+1))
			done
			...
			MIDDLEWARENAME="./mw_$NUM_SERVERS-$ROUND-$REPLICATION.log"
