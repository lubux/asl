...
SERVER_PORT=11500
SERVER_MACHINE_P="10.0.0.8:$SERVER_PORT 10.0.0.14:$SERVER_PORT 10.0.0.12:$SERVER_PORT 10.0.0.7:$SERVER_PORT 10.0.0.10:$SERVER_PORT 10.0.0.4:$SERVER_PORT 10.0.0.5:$SERVER_PORT"
INSTALL=0
NUM_MINUTES="3m"
LOG_ITER="1s"
...
ROUND_DIRS_NAME="round"
OUTPUT_DIR="writesexp"
#WORKLOAD_FILES="writes1.cfg writes5.cfg writes10.cfg"
WORKLOAD_FILES="writes1.cfg writes5.cfg writes10.cfg"
...
for ROUND in {1,2,3,4,5}
do
	ROUND_DIR_NAME=$ROUND_DIRS_NAME$ROUND
	mkdir ../data/$OUTPUT_DIR/$ROUND_DIR_NAME
		...
		do
			...
			do
				...
				do
					...
					MEMASLAP_CMD="./libmemcached-1.0.18/clients/memaslap -s $MW_MACHINE_P:$SERVER_PORT -T $NUM_CLIENTS -c $NUM_CLIENTS -o 0.9 -S 1s -t $NUM_MINUTES -F $WORKLOAD > clientlog_$NUM_SERVERS-$COUNTER-$WORKLOAD_COUNTER-$REPLICATION.txt"
					echo $MEMASLAP_CMD
				...
				for client_machine in $CLIENT_MACHINES
				do
					scp $USER_MACHINES@$client_machine:clientlog_$NUM_SERVERS-$COUNTER-$WORKLOAD_COUNTER-$REPLICATION.txt ../data/$OUTPUT_DIR/$ROUND_DIR_NAME
					ssh $USER_MACHINES@$client_machine "rm clientlog_$NUM_SERVERS-$COUNTER-$WORKLOAD_COUNTER-$REPLICATION.txt"
					COUNTER=$((COUNTER+1))
				done
				...
				MIDDLEWARENAME="./mw_$NUM_SERVERS-$WORKLOAD_COUNTER-$REPLICATION.log"
				ssh $USER_MACHINES@$MW_MACHINE "mv ./mw.log $MIDDLEWARENAME"
				scp $USER_MACHINES@$MW_MACHINE:$MIDDLEWARENAME ../data/$OUTPUT_DIR/$ROUND_DIR_NAME
	...
	telegram-cli -W -e "msg Lubu 'Writes Experiment Round $ROUND Finished'" &
done
